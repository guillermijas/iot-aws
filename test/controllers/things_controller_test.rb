...
class ThingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create thing' do
    assert_difference('Thing.count') do
      post things_url, params: { thing: { desired_value: @thing.desired_value, name: @thing.name, status: @thing.status } }
    end
  ...
  test 'should update thing' do
    patch thing_url(@thing), params: { thing: { desired_value: @thing.desired_value, name: @thing.name, status: @thing.status } }
